...
async function mergeRecentSessions(stats: StatsCache): Promise<StatsCache> {
  ...
  // Deep copy existing aggregate maps for enrichment
  const toolFrequency: Record<string, number> = { ...(stats.toolFrequency ?? {}) }
  const agentDispatches: Record<string, number> = { ...(stats.agentDispatches ?? {}) }
  const skillInvocations: Record<string, number> = { ...(stats.skillInvocations ?? {}) }
  let planModeCount = stats.planModeCount ?? 0
  ...
  for (const s of recentSessions) {
    ...
    if (detail) {
      ...
      for (const [tool, count] of Object.entries(detail.toolFrequency)) {
        toolFrequency[tool] = (toolFrequency[tool] ?? 0) + count
      }

      // Aggregate agent dispatches
      for (const agent of detail.agents) {
        agentDispatches[agent.subagentType] = (agentDispatches[agent.subagentType] ?? 0) + 1
      }

      // Aggregate skill invocations
      for (const skill of detail.skills) {
        skillInvocations[skill.skill] = (skillInvocations[skill.skill] ?? 0) + 1
      }
      for (const agent of detail.agents) {
      ...
      // Count plan mode
      planModeCount += (detail.toolFrequency['EnterPlanMode'] ?? 0)
  ...
  try {
    ...
    for (const s of summaries) {
      ...
      if (detail) {
        ...
        // Aggregate agent dispatches by type
        for (const agent of detail.agents) {
          agentDispatches[agent.subagentType] = (agentDispatches[agent.subagentType] ?? 0) + 1
        }

        // Aggregate skill invocations (session-level + agent-level)
        for (const skill of detail.skills) {
          skillInvocations[skill.skill] = (skillInvocations[skill.skill] ?? 0) + 1
        }
        for (const agent of detail.agents) {
